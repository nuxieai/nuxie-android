...
Three consecutive API 36 ARM64 SwiftShader / TalkBack 16.0.0.738667889 runs passed on Android `e5d91cc` production code (`/tmp/nuxie-talkback-home-return-current-tree.log`). Each driver restored and verified settings and notification permission/flags. The initial harness needed two corrections: import its assertion, and compare the resumed virtual control against the newly published node rather than a retired pre-background platform object. Both adapters withdraw virtual objects while inactive; the native editor retains its identity. Earlier assertion failures are harness evidence, not product regressions. The final test preserves the existing two-second swipe deadline and bounds focus restoration at five seconds.
