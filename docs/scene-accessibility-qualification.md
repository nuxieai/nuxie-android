...
## TalkBack initial native-load recovery

`--scenario native-retry` exercises `PublishedTextInputDeviceTest.talkBackRetriesInitialNativeFailure`: a real native import fails inside the authenticated presentation, TalkBack reaches and activates Retry, and the replacement attempt reveals once in the same Activity/container with one screen appearance and no remaining recovery view. The failed attempt uses the public runtime and real native ownership, not a renderer double.
