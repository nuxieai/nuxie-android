...
class JourneyReleaseEnvelopeTest {
    ...
    @Test fun `both Journey envelopes authenticate their exact publisher bytes`() {
        for (entry in listOf("entry", "renderedEntry")) {
            val envelope = envelope(entry)
            val verified = JourneyReleaseEnvelope.authenticate(
    ...
    @Test fun `noncanonical media type is rejected`() {
        val changed = JsonObject(envelope() + ("mediaType" to JsonPrimitive("application/json")))
        assertThrows(JourneyReleaseAuthenticationException::class.java) {
            JourneyReleaseEnvelope.authenticate(changed.toString().encodeToByteArray(), keys)
        }
    }
    ...
    @Test fun `integral JSON numbers do not require a particular lexical spelling`() {
        ...
        val verified = JourneyReleaseEnvelope.authenticate(changed.toString().encodeToByteArray(), keys)
    ...
    @Test fun `duplicate envelope keys are rejected after JSON escape decoding`() {
        for (duplicate in listOf("mediaType", "media\\u0054ype")) {
            val text = envelope().toString().replaceFirst("{", "{\"$duplicate\":\"application/vnd.nuxie.journey+json\",")
            assertThrows(JourneyReleaseAuthenticationException::class.java) {
                JourneyReleaseEnvelope.authenticate(text.encodeToByteArray(), keys)
            }
        }
    }
